...
pub fn parse(input: ParseStream<'_>) -> Result<Vec<Path>> {
    let content;
    ...
    for entry in entries {
        if entry.qself.is_some() {
            return Err(syn::Error::new_spanned(
                entry,
                "event paths cannot use qualified self syntax",
            ));
        }
                ...
                })
        {
            return Err(syn::Error::new_spanned(
                arguments,
                "event paths cannot contain generic arguments",
            ));
        }
        let lexical_path = entry.path.to_token_stream().to_string();
        if !lexical_paths.insert(lexical_path) {
            return Err(syn::Error::new_spanned(&entry.path, "duplicate event path"));
        }
        events.push(entry.path);
    }
    Ok(events)
}
