...
pub fn parse(tokens: TokenStream) -> syn::Result<Input> {
    ...
    if item.trait_.is_some() {
        return Err(syn::Error::new_spanned(
            &item,
            "domain_decisions only supports inherent impl blocks",
        ));
    }
    if !item.generics.params.is_empty() || item.generics.where_clause.is_some() {
        return Err(syn::Error::new_spanned(
            &item.generics,
            "domain_decisions does not support generic impl blocks or impl where clauses",
        ));
    }
    let Type::Path(owner) = item.self_ty.as_ref() else {
        return Err(syn::Error::new_spanned(
            &item.self_ty,
            "domain_decisions requires a concrete path self type",
        ));
    };
