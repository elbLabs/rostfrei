...
fn main() -> Result<(), Box<dyn Error>> {
    ...
    write_schema(
        &schema_directory.join("behavioral-test-v2.schema.json"),
        &behavioral_test_definition_schema(),
    )?;
    write_schema(
        &schema_directory.join("message-series-definition-v2.schema.json"),
        &message_series_definition_schema(),
    )?;
    write_schema(
        &schema_directory.join("observed-message-series-v2.schema.json"),
        &observed_message_series_schema(),
    )?;
    Ok(())
}
