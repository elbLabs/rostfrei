...
impl RolePolicy {
    pub(super) fn for_file(file: &SourceFileFacts) -> Option<Self> {
        ...
        let policy = match file_name {
            ...
            "policy.rs" => Self::Contract(PrimaryKind::Policy),
            "outcome.rs" => Self::Declaration(PrimaryKind::PolicyOutcome),
    ...
    pub(super) fn allows(self, item: &TopLevelItem, file: &SourceFileFacts) -> bool {
        ...
        match self {
            ...
            Self::ValueObject => {
                item.kind == TopLevelItemKind::Nominal
                    && item.primaries.contains(&PrimaryKind::ValueObject)
                    || item.kind == TopLevelItemKind::Implementation
                        && item.trait_name.is_none()
                        && item.self_type.as_deref() == primary_name(file, PrimaryKind::ValueObject)
            }
            Self::EventSet => {
                item.kind == TopLevelItemKind::Nominal
