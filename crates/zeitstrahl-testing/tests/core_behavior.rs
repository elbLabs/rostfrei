...
use zeitstrahl_core::{
    Aggregate, AggregateId, AggregateType, AppendOutcome, CommandHandler, ContentFingerprint,
    DecisionContext, EnvelopeError, EventBatch, EventCodec, EventCodecError, EventCodecErrorKind,
...
use zeitstrahl_testing::{event_store_contract, given};
...
impl EventCodec<Account> for AccountCodec {
    fn encode(
        &self,
        event: &AccountEvent,
        event_id: zeitstrahl_core::EventId,
    ...
    fn decode(
        &self,
        event: &zeitstrahl_core::RecordedEvent,
...
async fn executor_replays_retries_rejections_and_preserves_import_provenance() {
    ...
    let retried = executor
        .execute::<Account, _>(credit_metadata, &credit)
        .await
...
fn envelopes_and_identities_reject_invalid_values() {
    ...
    assert_eq!(
        EventBatch::new(
            zeitstrahl_core::CommitId::new("commit").expect("valid id"),
