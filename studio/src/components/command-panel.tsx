...
export function CommandPanel({
  ...
  const testAvailable = selected
    ? Boolean(selected.version.testHrefTemplate)
    : choices.some((choice) => choice.version.testHrefTemplate !== undefined)
  // Keep the user's Test preference when switching through Preview-only commands.
  const executionMode = testAvailable ? mode : "preview"
        ...
        <form className="command-form" onSubmit={submit}>
          <div className="command-mode-row">
            ...
            >
              <button
                ...
                aria-pressed={executionMode === "preview"}
                disabled={busy}
                onClick={() => {
                  setMode("preview")
                  markEdited()
            ...
            <p>
              {executionMode === "test"
                ...
                : "Reads the connected event history. Nothing is published or appended."}
            </p>
          </div>
          ...
          <Button
            ...
            disabled={
              ...
              inputsLoading ||
              !selected ||
              (executionMode === "test" && !selected.version.testHrefTemplate)
            }
          >
            {busy ? (
...
function PayloadField({
  ...
  const selectionInput =
    identifier &&
    input?.options.every((option) => typeof option.value === "string")
      ? undefined
      : input
...
function buildPayload(
  ...
  const properties = fields.map((field) => {
    ...
    const input = fieldInput(field, advertisedInput)
      ...
      ? `[${fields
          .map((field) =>
            encodeField(
              ...
              fieldInput(field, inputsByName.get(field.name)),
              field.name
            )
          )
          .join(",")}]`
  ...
  return Object.fromEntries(
    fields.flatMap((field) => {
      ...
      if (isIdentifierField(field)) {
        if (fieldInput(field, input)) return [[field.name, "option:0"]]
...
function fieldInput(
  field: CatalogCommandField,
  input?: CommandInputField
): CommandInputField | undefined {
  return isIdentifierField(field) &&
    input?.options.every((option) => typeof option.value === "string")
    ? undefined
    : input
}
