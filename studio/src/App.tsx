...
function isEditableNumericScalar(scalar: string | undefined) {
  return scalar !== undefined && [
...
function App() {
  ...
  const commandInputComplete = parsedPayload.payload !== null && commandFields.every((field) => {
    if (field.value.kind === "optional") return true;
                    ...
                    <div className="generated-inputs">
                      ...
                      {!inputsLoading && commandFields.map((field) => {
                        const discovered = commandInputs?.fields.find((item) => item.name === field.name);
                         ...
                         const optionsAreBrowserSafe = options.every((option) => isBrowserSafeJson(option.value));
                         ...
                         const optional = field.value.kind === "optional";
                         const inputValue = optional && field.value.value ? field.value.value : field.value;
                         const scalar = scalarName(inputValue.scalar);
                         const selectedChoice = currentValue === null || currentValue === undefined
                           ? (optional ? "unset" : "")
                           : `option:${optionValue(currentValue)}`;
                        return (
                          <div className="field-group generated-field" key={field.name}>
                            <Label>{discovered?.label ?? field.name}</Label>
                            {options.length && optionsAreBrowserSafe ? (
                              <Select
                                value={selectedChoice}
                                onValueChange={(value) => {
                                  if (value === "unset") {
                                    setPayloadField(field.name, null);
                                    return;
                                  }
                                  const option = options.find((item) => `option:${optionValue(item.value)}` === value);
                              ...
                              >
                                ...
                                <SelectContent>{optional && <SelectItem value="unset">Unset</SelectItem>}{options.map((option) => <SelectItem key={optionValue(option.value)} value={`option:${optionValue(option.value)}`}>{option.label}</SelectItem>)}</SelectContent>
                              </Select>
                            ) : scalar === "bool" ? (
                              <Select value={currentValue === null || currentValue === undefined ? "unset" : String(currentValue)} onValueChange={(value) => setPayloadField(field.name, value === "unset" ? null : value === "true")}>
                                <SelectTrigger><SelectValue /></SelectTrigger>
                                <SelectContent>{optional && <SelectItem value="unset">Unset</SelectItem>}<SelectItem value="true">true</SelectItem><SelectItem value="false">false</SelectItem></SelectContent>
                              </Select>
                            ) : inputValue.kind === "scalar" && isEditableNumericScalar(scalar) ? (
                              <Input type="number" step={scalar === "f32" || scalar === "f64" ? "any" : "1"} value={typeof currentValue === "number" ? currentValue : ""} onChange={(event) => setPayloadField(field.name, event.target.value === "" ? (optional ? null : "") : Number(event.target.value))} />
                            ) : inputValue.kind === "scalar" && isNumericScalar(scalar) ? (
                              <div className="input-unavailable"><Braces size={16} /> Edit this integer in the JSON payload below to preserve its exact value.</div>
                            ) : inputValue.kind === "scalar" ? (
                              <Input value={typeof currentValue === "string" ? currentValue : ""} onChange={(event) => setPayloadField(field.name, event.target.value === "" && optional ? null : event.target.value)} />
                            ) : (
                              <div className="input-unavailable"><Braces size={16} /> Edit this value in the JSON payload below.</div>
                            )}
                            {selectedOption?.description && <span className="input-description"><CircleCheck size={13} /> {selectedOption.description}</span>}
                            {!optionsAreBrowserSafe && <span className="input-description"><Braces size={13} /> Runtime choices include integers outside the browser's exact range; edit the raw JSON payload instead.</span>}
