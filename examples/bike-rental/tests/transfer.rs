...
use rostfrei::{
    CommandBus, CommandMessageAdapter, CommandOutcome, CommandReceipt, CommandRequest,
    ContentFingerprint, DomainEvent, EventStore, ExecutionMetadata, Executor, OperationId,
    SimulationDecision,
};
...
async fn executor_atomically_transfers_replays_and_rejects_in_memory() -> TestResult {
    ...
    let rejected = Executor::new(store.clone())
    ...
    let same_fleet = Executor::new(store.clone())
        .execute::<RentalFleetAggregate, _>(
...
async fn simulation_reports_both_participants_without_appending() -> TestResult {
    ...
    let outcome = Executor::new(store.clone())
        .simulate::<RentalFleetAggregate, _>(
        ...
        matches!(outcome.decision(), SimulationDecision::Accepted(events) if events.len() == 1),
    ...
    let [source, destination] = outcome.participants() else {
        ...
        "simulation primary participant is not the source transfer event",
    )?;
    ensure(
...
async fn rejected_simulation_reports_the_loaded_destination_as_a_read_participant() -> TestResult {
    ...
    let outcome = Executor::new(store)
        ...
        matches!(outcome.decision(), SimulationDecision::Rejected(rejection) if rejection.reason == BicycleTransferRejectionReason::SourceBicycleMissing),
...
async fn dispatch_transfer_through_nats(
    ...
    let receipt = command_bus
        .dispatch::<RentalFleetAggregate, TransferBicycle>(
            CommandRequest::new(
    ...
    S: EventStore + Clone,
{
    ...
    let first = executor
        .execute::<RentalFleetAggregate, _>(metadata.clone(), &command)
        .await?;
...
fn metadata(operation: &str) -> TestResult<ExecutionMetadata> {
